let grid = {
  ...
  createCollision:()=>{
      console.log("generating collision");
...
console.log(grid.collisionNumbersArray);
  ...
  create: () => {
    function dot(x, y) {
      ...
      this.collision = {x1:0,x2:0,y1:0,y2:0};
      this.active = false;
      this.createSelf = (that) => {
        ...
        that.body.style.width = (grid.size.x/grid.dotCount)*.13 + "%";
        that.body.style.height = (grid.size.y/grid.dotCount)*.13 + "%";
  ...
  pullData:(query)=>{
    ...
    $(document).ready(function() {
    $.ajax({
       url : "gridPhp/pullDots.php?code="+encodeURIComponent('ToUVxce7hxex1pnTvJmO8PKyXXQlLzvp'),
       type : 'GET',
      ...
      }else if(document.getElementById("canvasPage") != null){
      }else{
                $(document).ready(function() {
                $.ajax({
                    url : "gridPhp/pullDots.php?code="+encodeURIComponent(query),
                    type : 'GET',
                    success : function(result){ 
                    grid.pattern(JSON.parse(result)); 
                },
                    error: function(){
                        setTimeout(()=>{
                        grid.pullData();
